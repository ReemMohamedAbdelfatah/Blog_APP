...
RSpec.describe 'Routes', type: :request do
  ...
  describe 'GET index' do
    let(:user) { User.create(name: 'user', photo: 'photo', bio: 'bio', Posts_Counter: 0) }
  ...
  describe 'GET /show' do
    let(:user) { User.create(name: 'user', photo: 'photo', bio: 'bio', Posts_Counter: 0) }
    let(:post) do
      Post.create(author: user, title: 'title', text: 'description', likes_counter: 0, comments_counter: 0)
    end
